...
public class EvidenceText : MonoBehaviour
{
    ...
    public void ChangeEvidenceText(string location)
    {
        ...
        {
            case "park":
                locationindex = 0; break;
            case "hotel":
                locationindex = 1; break;
            case "lake":
                locationindex = 2; break;
            case "stadium":
                locationindex = 3; break;
            case "train station":
                locationindex = 4; break;
            default: 
                locationindex = 0; break;
        }

        for (int i=0; i<inventoryManager.allEvidence.Length; i++)
        {
            if (inventoryManager.allEvidence[i].locationIndex == locationindex && inventoryManager.inventory[i])
            {
                displayText += inventoryManager.allEvidence[i].name + ":\n" + inventoryManager.allEvidence[i].interactions[0].textResponse + "\n\n";
            }
        }
        ...
        if (displayText.Length == 0) displayText = "No evidence found at this location.";
